# 第14章：你的第一个 3D模型

我在想给你理解 Blender 建模最好的第一个模型是什么，决定从棋子兵开始。

最后看到自己能建模出来时，既有趣又满足。

让我们开始吧！

首先，我们找一张参考图。

我在 Pixabay.com 上找到的。（https://pixabay.com/vectors/chess-game-pawn-piece-recreation-26779/）。

你可以下载这张或任何你想要的图片，只要确保是正面视图。我会在描述里放上图片下载链接。

![page_92_img_1.png](../images/page_92_img_1.png)

现在来学习如何用图片参考建模。我们目前在物体模式。

首先，用"X"删除场景里的所有东西。

![page_92_img_2.png](../images/page_92_img_2.png)

然后，点击 gizmo 上的"-Y"，这样从一开始就能正确定位。-Y 是正面。

![page_93_img_1.png](../images/page_93_img_1.png)

然后去 `Add → Image → Reference`。

之后会打开新窗口，你找到刚才下载的图片。

![page_93_img_2.png](../images/page_93_img_2.png)

选择图片，点击"Add Image"按钮。

![page_94_img_1.png](../images/page_94_img_1.png)

图片添加好了。

![page_94_img_2.png](../images/page_94_img_2.png)

右边你会看到很多工具，但现在对我们重要的是 `Data → Object` 数据属性。

![page_94_img_3.png](../images/page_94_img_3.png)

这里你可以改变图片的大小、偏移、深度、边和透明度。

我们要把 Depth 改成 Back。

![page_95_img_1.png](../images/page_95_img_1.png)

下一步是把兵放在中心，让兵的底部和 X轴对齐，方便建模。

因为参考图已经在中间了，我只要用"`G+Z`"把它往上移大约 2.29，让底部和 X轴对齐。

![page_95_img_2.png](../images/page_95_img_2.png)

添加物体之前，我们先想想根据兵的形状，哪个物体最适合建模。

我们有：平面、立方体、圆、UV球体、Ico球体、圆柱、圆锥和圆环。

最相似的是圆和圆柱。

你可以选任意一个。我用圆开始。

去 `Add → Mesh → Circle`

![page_96_img_1.png](../images/page_96_img_1.png)

用"TAB"切换到编辑模式

![page_96_img_2.png](../images/page_96_img_2.png)

按"F"填充

![page_96_img_3.png](../images/page_96_img_3.png)

点击"-Y"切换到正交前视图

![page_97_img_1.png](../images/page_97_img_1.png)

往上滚动鼠标滚轮，放大兵的右侧。

![page_97_img_2.png](../images/page_97_img_2.png)

现在我们开始挤出和缩放圆形，沿着兵的轮廓。

用"S"缩放到大约 1.63

![page_97_img_3.png](../images/page_97_img_3.png)

用"`E+Z`"挤出大约 0.05

![page_98_img_1.png](../images/page_98_img_1.png)

用"S"向外缩放大约 1.04

![page_98_img_2.png](../images/page_98_img_2.png)

用"`E+Z`"挤出大约 0.05

![page_98_img_3.png](../images/page_98_img_3.png)

用"S"向外缩放大约 1.015

![page_99_img_1.png](../images/page_99_img_1.png)

用"`E+Z`"挤出大约 0.2764

![page_99_img_2.png](../images/page_99_img_2.png)

用"`E+Z`"挤出大约 0.05

![page_99_img_3.png](../images/page_99_img_3.png)

用"S"向内缩放大约 0.985

![page_100_img_1.png](../images/page_100_img_1.png)

用"`E+Z`"挤出大约 0.05

![page_100_img_2.png](../images/page_100_img_2.png)

用"S"向内缩放大约 0.95

![page_100_img_3.png](../images/page_100_img_3.png)

当你到达第一个褶皱时，挤出圆一点点然后缩放，让它贴合褶皱内部。

用"`E+Z`"挤出大约 0.005

![page_101_img_1.png](../images/page_101_img_1.png)

用"S"向内缩放大约 0.9

![page_101_img_2.png](../images/page_101_img_2.png)

用"`E+Z`"挤出大约 0.05

![page_101_img_3.png](../images/page_101_img_3.png)

用"S"向外缩放大约 1.025

![page_102_img_1.png](../images/page_102_img_1.png)

用"`E+Z`"挤出大约 0.05

![page_102_img_2.png](../images/page_102_img_2.png)

用"S"向外缩放大约 1.005

![page_102_img_3.png](../images/page_102_img_3.png)

用"`E+Z`"挤出大约 0.47

![page_103_img_1.png](../images/page_103_img_1.png)

用"`E+Z`"挤出大约 0.05

![page_103_img_2.png](../images/page_103_img_2.png)

用"S"向内缩放大约 0.985

![page_103_img_3.png](../images/page_103_img_3.png)

用"`E+Z`"挤出大约 0.05

![page_104_img_1.png](../images/page_104_img_1.png)

用"S"向内缩放大约 0.96

![page_104_img_2.png](../images/page_104_img_2.png)

接下来是兵的下一部分。像上次一样，我们用"`E+Z`"挤出一点点，这样之后可以缩放。

用"`E+Z`"挤出大约 0.005

![page_105_img_1.png](../images/page_105_img_1.png)

用"S"向内缩放大约 0.86

![page_105_img_2.png](../images/page_105_img_2.png)

用"`E+Z`"挤出大约 0.05

![page_105_img_3.png](../images/page_105_img_3.png)

用"S"向外缩放大约 1.035

![page_106_img_1.png](../images/page_106_img_1.png)

用"`E+Z`"挤出大约 0.05

![page_106_img_2.png](../images/page_106_img_2.png)

用"S"向外缩放大约 1.008

![page_106_img_3.png](../images/page_106_img_3.png)

用"`E+Z`"挤出大约 0.05

![page_106_img_4.png](../images/page_106_img_4.png)

用"S"向内缩放大约 0.995

![page_107_img_1.png](../images/page_107_img_1.png)

用"`E+Z`"挤出大约 0.05

![page_107_img_2.png](../images/page_107_img_2.png)

用"S"向内缩放大约 0.97

![page_107_img_3.png](../images/page_107_img_3.png)

用"`E+Z`"挤出大约 0.025

![page_108_img_1.png](../images/page_108_img_1.png)

用"S"向内缩放大约 0.96

![page_108_img_2.png](../images/page_108_img_2.png)

又是同样的事。只挤出一点点，这样我们可以缩放下一部分。

用"`E+Z`"挤出大约 0.005

![page_108_img_3.png](../images/page_108_img_3.png)

用"S"向内缩放大约 0.82

![page_109_img_1.png](../images/page_109_img_1.png)

这次我们可以直接挤出这部分。

用"`E+Z`"挤出大约 1.11

![page_109_img_2.png](../images/page_109_img_2.png)

用"S"向内缩放大约 0.76

![page_109_img_3.png](../images/page_109_img_3.png)

之后，下一部分要来了，只挤出一点点。

用"`E+Z`"挤出大约 0.005

![page_110_img_1.png](../images/page_110_img_1.png)

用"S"向外缩放大约 1.6

![page_110_img_2.png](../images/page_110_img_2.png)

用"`E+Z`"挤出大约 0.05

![page_110_img_3.png](../images/page_110_img_3.png)

用"S"向外缩放大约 1.06

![page_110_img_4.png](../images/page_110_img_4.png)

用"`E+Z`"挤出大约 0.05

![page_111_img_1.png](../images/page_111_img_1.png)

用"S"向外缩放大约 1.02

![page_111_img_2.png](../images/page_111_img_2.png)

用"`E+Z`"挤出大约 0.05

![page_111_img_3.png](../images/page_111_img_3.png)

用"`E+Z`"挤出大约 0.05

![page_111_img_4.png](../images/page_111_img_4.png)

用"S"向内缩放大约 0.975

![page_112_img_1.png](../images/page_112_img_1.png)

用"`E+Z`"挤出大约 0.05

![page_112_img_2.png](../images/page_112_img_2.png)

用"S"向内缩放大约 0.94

![page_112_img_3.png](../images/page_112_img_3.png)

褶皱处又是同样的操作。

用"`E+Z`"挤出大约 0.005

![page_112_img_4.png](../images/page_112_img_4.png)

用"S"向内缩放大约 0.6

![page_113_img_1.png](../images/page_113_img_1.png)

用"`E+Z`"挤出大约 0.1

![page_113_img_2.png](../images/page_113_img_2.png)

用"S"向外缩放大约 1.14

![page_113_img_3.png](../images/page_113_img_3.png)

用"`E+Z`"挤出大约 0.1

![page_113_img_4.png](../images/page_113_img_4.png)

用"S"向外缩放大约 1.1

![page_114_img_1.png](../images/page_114_img_1.png)

用"`E+Z`"挤出大约 0.1

![page_114_img_2.png](../images/page_114_img_2.png)

用"S"向外缩放大约 1.07

![page_114_img_3.png](../images/page_114_img_3.png)

用"`E+Z`"挤出大约 0.1

![page_115_img_1.png](../images/page_115_img_1.png)

用"S"向外缩放大约 1.05

![page_115_img_2.png](../images/page_115_img_2.png)

用"`E+Z`"挤出大约 0.1

![page_115_img_3.png](../images/page_115_img_3.png)

用"S"向外缩放大约 1.03

![page_116_img_1.png](../images/page_116_img_1.png)

用"`E+Z`"挤出大约 0.1

![page_116_img_2.png](../images/page_116_img_2.png)

用"S"向外缩放大约 1.02

![page_116_img_3.png](../images/page_116_img_3.png)

用"`E+Z`"挤出大约 0.1

![page_117_img_1.png](../images/page_117_img_1.png)

用"S"向外缩放大约 1.01

![page_117_img_2.png](../images/page_117_img_2.png)

用"`E+Z`"挤出大约 0.1

![page_117_img_3.png](../images/page_117_img_3.png)

用"`E+Z`"挤出大约 0.1

![page_117_img_4.png](../images/page_117_img_4.png)

用"S"向外缩放大约 0.995

![page_118_img_1.png](../images/page_118_img_1.png)

用"`E+Z`"挤出大约 0.1

![page_118_img_2.png](../images/page_118_img_2.png)

用"S"向内缩放大约 0.98

![page_118_img_3.png](../images/page_118_img_3.png)

用"`E+Z`"挤出大约 0.1

![page_119_img_1.png](../images/page_119_img_1.png)

用"S"向内缩放大约 0.975

![page_119_img_2.png](../images/page_119_img_2.png)

用"`E+Z`"挤出大约 0.1

![page_119_img_3.png](../images/page_119_img_3.png)

用"S"向内缩放大约 0.96

![page_119_img_4.png](../images/page_119_img_4.png)

用"`E+Z`"挤出大约 0.1

![page_120_img_1.png](../images/page_120_img_1.png)

用"S"向内缩放大约 0.95

![page_120_img_2.png](../images/page_120_img_2.png)

用"`E+Z`"挤出大约 0.1

![page_120_img_3.png](../images/page_120_img_3.png)

用"S"向内缩放大约 0.92

![page_121_img_1.png](../images/page_121_img_1.png)

用"`E+Z`"挤出大约 0.1

![page_121_img_2.png](../images/page_121_img_2.png)

用"S"向内缩放大约 0.9

![page_121_img_3.png](../images/page_121_img_3.png)

用"`E+Z`"挤出大约 0.1

![page_122_img_1.png](../images/page_122_img_1.png)

用"S"向内缩放大约 0.85

![page_122_img_2.png](../images/page_122_img_2.png)

用"`E+Z`"挤出大约 0.1

![page_122_img_3.png](../images/page_122_img_3.png)

用"S"向内缩放大约 0.75

![page_122_img_4.png](../images/page_122_img_4.png)

用"`E+Z`"挤出大约 0.1

![page_123_img_1.png](../images/page_123_img_1.png)

用"S"向内缩放大约 0.4

![page_123_img_2.png](../images/page_123_img_2.png)

用"`E+Z`"挤出大约 0.02

![page_123_img_3.png](../images/page_123_img_3.png)

按"M"合并到中心。

![page_124_img_1.png](../images/page_124_img_1.png)

![page_124_img_2.png](../images/page_124_img_2.png)

用"TAB"切换到物体模式

![page_124_img_3.png](../images/page_124_img_3.png)

右键选择 Shade Auto Smooth。

![page_125_img_1.png](../images/page_125_img_1.png)

今天就到这里，下次我们学习怎么做盘子。

如果你和我一起学习觉得有趣，别忘了订阅我的频道。